...
const CONFIGURED = process.env.NEXT_PUBLIC_LAB_URL;

export const LAB_URL = CONFIGURED || "http://localhost:8000";

/** Whether a service URL was deliberately configured for this deployment. */
export const LAB_CONFIGURED = Boolean(CONFIGURED);

const TIMEOUT_MS = 20000;

async function call(path, init) {
  // No configured service: do not probe. An https page calling
  // http://localhost is blocked as mixed content and would log an error on
  // every visit, for a service the page does not need.
  if (!CONFIGURED) {
    return { ok: false, offline: true, unconfigured: true, error: "No lab service is configured for this deployment." };
  }
