...
/**
 * The one place a vector id becomes display text.
 *
 * Deriving labels from the id — `attackType.replace(/_/g, " ")` plus CSS
 * `capitalize` — produced "Qr Swap" for what the taxonomy calls "QR
 * substitution", and Title Case everywhere the rest of the product uses
 * sentence case. Worse, the planner panel showed the canonical label in its
 * heading and the derived one in its own bar list, so the same vector carried
 * two different names eight pixels apart.
 *
 * Every surface that renders a vector name calls this.
 */
const ACRONYMS = new Set([
  "qr", "kyc", "otp", "sim", "upi", "ach", "atm", "api", "ai", "id", "bec",
  "imps", "neft", "rtgs", "swift", "p2p", "3ds", "sms",
]);

/** Only reached for an id with no taxonomy entry. Still respects acronyms. */
...
export const labelFor = (id) => vectorById(id)?.name || fallbackLabel(id);
