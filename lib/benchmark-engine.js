...
export function benchmark(seed = 2026, perFamily = 3, legitPerCustomer = 30) {
...
// Families an ordinary detector already handles, versus the evasive ones the
// adversarial rounds exist to teach it.
const EASY = ["account_takeover", "video_deepfake", "device_switch"];
const EVASIVE = ["sleeper_transaction_pacing", "behavioral_drift", "adversarial_probing", "phishing", "vishing", "velocity_anomaly", "synthetic_identity"];
...
export function train(seed = null) {
  ...
  const easyAll = fraudFrom(EASY, rng, 3, "easy");
  const evasiveAll = fraudFrom(EVASIVE, rng, 2, "medium");
  const hardAll = fraudFrom(EVASIVE, rng, 2, "hard");
