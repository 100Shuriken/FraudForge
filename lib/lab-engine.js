...
import { VECTORS, labelFor } from "./taxonomy.js";
...
function explain(decision, attackType) {
  ...
  return `Missed at ${decision.score.toFixed(2)}, below the ${REVIEW_THRESHOLD.toFixed(2)} review line. ${reasons}. Extracted as a ${labelFor(attackType)} evasion sample.`;
}
  ...
  return {
    ...
    plan: {
      ...
      rationale:
        ...
        `of the time. The planner scored ${plan.label} highest at ` +
